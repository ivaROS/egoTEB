...
namespace teb_local_planner
{
  ...
  void EdgeGap::computeError()
  {
    ...
    ROS_DEBUG_STREAM("Gap: [" << _measurement[0].x << "," << _measurement[0].y <<"] - [" << _measurement[0].x << "," << _measurement[0].y << "]. Pose: (" << pose->pose().position().x() << "," << pose->pose().position().y() << ") Costl: " << costl << ", Costr: " << costr);
